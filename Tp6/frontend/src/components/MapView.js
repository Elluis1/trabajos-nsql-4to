...
const MapView = () => {
  ...
  const handleDeleteAirport = async (iata, icao) => {
    const code = iata || icao;

    if (!window.confirm(`¿Seguro que querés eliminar el aeropuerto ${code}?`)) {
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.detail || "Error al eliminar aeropuerto");
      }

      // Sacarlo del mapa
      setAirports((prev) =>
        prev.filter((a) => a.iata_faa !== code && a.icao !== code)
      );

      console.log("🗑️ Aeropuerto eliminado:", code);
    } catch (err) {
      alert("❌ " + err.message);
    }
  };
          ...
          >
            <Popup>
              <div className="popup-content">
                ...
                <div className="popup-row">
                  <span className="popup-label">🗺️ Código IATA:</span>{" "}
                  {a.iata_faa || "N/A"}
                </div>
                <div className="popup-row">
                ...
                <button
                  ...
                  onClick={() => handleDeleteAirport(a.iata_faa, a.icao)}
                >
                  🗑️ Eliminar aeropuerto
                </button>
              </div>
            </Popup>
          </Marker>
